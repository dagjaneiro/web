...
export function homeCtrl(
  ...
  const initiateSync = () => {
    ...
    this.syncStatusObserver = syncManager.registerSyncStatusObserver(status => {
      if (status.retrievedCount > 20) {
        ...
        this.uploadSyncStatus = statusManager.replaceStatusWithString(
          this.uploadSyncStatus,
          `Syncing ${status.current}/${status.total} items...`
        );
      } else if (this.uploadSyncStatus) {
        this.uploadSyncStatus = statusManager.removeStatus(
          this.uploadSyncStatus
        );
      }
    });
    ...
    syncManager.addEventHandler((syncEvent, data) => {
      ...
      if (syncEvent == 'sync-session-invalid') {
        ...
        if (
          !lastSessionInvalidAlert ||
          (new Date() - lastSessionInvalidAlert) / 1000 > showInterval
        ) {
    ...
    const encryptionEnabled = authManager.user || passcodeManager.hasPasscode();
    this.syncStatus = statusManager.addStatusFromString(
  ...
  async function autoSignInFromParams() {
    ...
    if (!authManager.offline()) {
      ...
      if (
        (await syncManager.getServerURL()) === server &&
