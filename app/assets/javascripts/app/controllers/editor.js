...
export function editorCtrl(
  ...
  this.emptyTrash = function() {
    ...
    if (
      confirm(`Are you sure you want to permanently delete ${count} note(s)?`)
    ) {
  ...
  this.toggleProtectNote = function() {
    ...
    privilegesManager
      .actionHasPrivilegesConfigured(PrivilegesManager.ActionViewProtectedNotes)
      .then(configured => {
        if (!configured) {
  ...
  */
  ...
  this.loadPreferences = function() {
    this.monospaceFont = authManager.getUserPrefValue(
      'monospaceFont',
      'monospace'
    );

    // On desktop application, disable spellcheck by default, as it is not performant.
    const defaultSpellcheckStatus = !isDesktopApplication();
    this.spellcheck = authManager.getUserPrefValue(
      'spellcheck',
      defaultSpellcheckStatus
    );

    this.marginResizersEnabled = authManager.getUserPrefValue(
    ...
    if (this.marginResizersEnabled) {
      const width = authManager.getUserPrefValue('editorWidth', null);
      if (width !== null) {
        this.leftResizeControl.setWidth(width);
        this.rightResizeControl.setWidth(width);
      }

      const left = authManager.getUserPrefValue('editorLeft', null);
      if (left !== null) {
  ...
  */
  ...
  this.reloadComponentStackArray = function() {
    this.componentStack = componentManager
      .componentsForArea('editor-stack')
      .sort((a, b) => {
  ...
  this.associateComponentWithCurrentNote = function(component) {
    component.disassociatedItemIds = component.disassociatedItemIds.filter(
      id => {
        return id !== this.note.uuid;
      }
    );
