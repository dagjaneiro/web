...
export function notesCtrl(
  ...
  this.loadPreferences = function() {
    ...
    this.hideNotePreview = authManager.getUserPrefValue(
      'hideNotePreview',
      false
    );
    ...
    const width = authManager.getUserPrefValue('notesPanelWidth');
    if (width) {
  ...
  this.onNoteRemoval = function() {
    const visibleNotes = this.visibleNotes();
    let index;
  ...
  this.resetPagination = function({ keepCurrentIfLarger } = {}) {
    const MinNoteHeight = 51.0; // This is the height of a note cell with nothing but the title, which *is* a display option
    this.DefaultNotesToDisplayValue =
      document.documentElement.clientHeight / MinNoteHeight || 20;
    if (
      keepCurrentIfLarger &&
      this.notesToDisplay > this.DefaultNotesToDisplayValue
    ) {
  ...
  this.selectNextNote = function() {
    var visibleNotes = this.visibleNotes();
    const currentIndex = visibleNotes.indexOf(this.selectedNote);
