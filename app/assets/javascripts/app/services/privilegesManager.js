...
export class PrivilegesManager extends SFPrivilegesManager {
    ...
    this.setDelegate({
      ...
      saveToStorage: async (key, value) => {
        return storageManager.setItem(
          key,
          value,
          storageManager.bestStorageMode()
        );
      },
  ...
  presentPrivilegesModal(action, onSuccess, onCancel) {
    ...
    var el = this.$compile(
      "<privileges-auth-modal action='action' on-success='onSuccess' on-cancel='onCancel' class='sk-modal'></privileges-auth-modal>"
    )(scope);
  ...
  presentPrivilegesManagementModal() {
    var scope = this.$rootScope.$new(true);
    var el = this.$compile(
      "<privileges-management-modal class='sk-modal'></privileges-management-modal>"
    )(scope);
    angular.element(document.body).append(el);
  }
