...
export class AuthManager extends SFAuthManager {
  ...
  async login(url, email, password, ephemeral, strictSignin, extraParams) {
    return super
      .login(url, email, password, strictSignin, extraParams)
      .then(response => {
        if (!response.error) {
  ...
  presentPasswordWizard(type) {
    ...
    scope.type = type;
    var el = this.$compile("<password-wizard type='type'></password-wizard>")(
      scope
    );
  ...
  setUserPrefValue(key, value, sync) {
    if (!this.userPreferences) {
      console.log('Prefs are null, not setting value', key);
      return;
    }
