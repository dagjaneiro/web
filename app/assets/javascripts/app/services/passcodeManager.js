...
export class PasscodeManager {
  constructor($rootScope, authManager, storageManager, syncManager) {
    ...
    this._hasPasscode =
      this.storageManager.getItemSync('offlineParams', StorageManager.Fixed) !=
      null;
  ...
  async setAutoLockInterval(interval) {
    return this.storageManager.setItem(
      PasscodeManager.AutoLockIntervalKey,
  ...
  async getAutoLockInterval() {
    const interval = await this.storageManager.getItem(
      PasscodeManager.AutoLockIntervalKey,
      StorageManager.FixedEncrypted
    );
    if (interval) {
  ...
  setPasscode(passcode, callback) {
    ...
    SFJS.crypto
      .generateInitialKeysAndAuthParamsForUser(uuid, passcode)
      .then(results => {
