...
export class AccountMenu {
  ...
  controller(
      ...
      $timeout(function() {
        authManager
          .login(
            ...
            $scope.formData.ephemeral,
            $scope.formData.strictSignin,
            extraParams
          )
          .then(response => {
            $timeout(() => {
    ...
    $scope.register = function() {
      const confirmation = $scope.formData.password_conf;
    ...
    $scope.clearDatabaseAndRewriteAllItems = function(
      alternateUuids,
      callback
    ) {
      storageManager.clearAllModels().then(() => {
    ...
    $scope.archiveFormData = {
      encrypted: !!$scope.encryptedBackupsAvailable()
    };
    $scope.user = authManager.user;
    ...
    $scope.importJSONData = function(data, password, callback) {
      ...
      if (data.auth_params) {
        SFJS.crypto
          .computeEncryptionKeysForUser(password, data.auth_params)
          .then(keys => {
            try {
              SFJS.itemTransformer
                .decryptMultipleItems(
                  data.items,
                  keys,
                  false
    ...
    */
    ...
    $scope.submitPasscodeForm = function() {
      ...
      const fn = $scope.formData.changingPasscode
        ? passcodeManager.changePasscode.bind(passcodeManager)
