export class SessionHistoryMenu {
  ...
  controller($scope, modelManager, sessionHistory, actionsManager, $timeout) {
    ...
    $scope.reloadHistory = function() {
      const history = sessionHistory.historyForItem($scope.item);
    ...
    $scope.openRevision = function(revision) {
      actionsManager.presentRevisionPreviewModal(
        revision.item.uuid,
    ...
    $scope.toggleDiskSaving = function() {
      if (!sessionHistory.diskEnabled) {
        if (
          !confirm(
            'Are you sure you want to save history to disk? This will decrease general performance, especially as you type. You are advised to disable this feature if you experience any lagging.'
          )
        ) {
          return;
        }
