...
export class PrivilegesManagementModal {
  ...
  controller(
    ...
    $scope.isCredentialRequiredForAction = function(action, credential) {
      ...
      return $scope.privileges.isCredentialRequiredForAction(
        action,
        credential
      );
    };
    ...
    $scope.reloadPrivileges = async function() {
      ...
      const sessionEndDate = await privilegesManager.getSessionExpirey();
      ...
      for (const cred of $scope.availableCredentials) {
        $scope.credentialDisplayInfo[cred] = $scope.displayInfoForCredential(
          cred
        );
      }
