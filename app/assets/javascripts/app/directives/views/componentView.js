...
export class ComponentView {
  ...
  controller(
    ...
    $scope.handleActivation = function() {
      ...
      const iframe = componentManager.iframeForComponent(component);
      if (iframe) {
        $scope.loading = true;
        ...
        iframe.onload = event => {
          ...
          $timeout.cancel($scope.loadTimeout);
          componentManager
            .registerComponentWindow(component, iframe.contentWindow)
            .then(() => {
    ...
    */
    ...
    $scope.reloadStatus = function(doManualReload = true) {
      ...
      const previouslyValid = $scope.componentValid;
      ...
      const urlError =
        ...
        (isDesktopApplication() &&
          (!component.local_url && !component.hasValidHostedUrl()));

      $scope.expired =
        component.valid_until && component.valid_until <= new Date();
